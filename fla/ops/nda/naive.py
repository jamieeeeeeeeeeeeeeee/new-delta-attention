...
def naive_chunk_nda(
    ...
    for i in range(BT):
        ...
        A[..., i] = torch.einsum('... c d, ... d -> ... c', k * (g - g_i).clamp(max=0).exp(), beta_i * k_i)
    #A = A * beta[..., None]
    ...
    A = (A + torch.eye(BT, dtype=torch.float, device=q.device)) # absorbed into BT loop
    ...
    for i in range(0, NT):
        # [B, H, BT, ...]
        q_i, k_i, u_i, g_i, w_i, beta_i = q[:, :, i], k[:, :, i], u[:, :, i], g[:, :, i], w[:, :, i], beta[:, :, i]
        ...
        for j in range(BT):
            ...
            A[..., j] = torch.einsum('... c d, ... d -> ... c', q_i * (g_i - g_j).clamp(max=0).exp(), beta_j * k_j)
        A = A.masked_fill(mask, 0)
        v_i = u_i - w_i @ S
        o[:, :, i] = (q_i * g_i.exp()) @ S + A @ v_i
        S = S * rearrange(g_i[:, :, -1].exp(), 'b h k -> b h k 1')
        #S += rearrange((g_i[:, :, -1:] - g_i).exp() * k_i, 'b h c k -> b h k c') @ v_i
        S += rearrange((g_i[:, :, -1:] - g_i).exp() * (beta_i * k_i), 'b h c k -> b h k c') @ v_i
    if not output_final_state:
        S = None
