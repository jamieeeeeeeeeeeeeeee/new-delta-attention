...
from fla.ops.nda.chunk_bwd import chunk_nda_bwd
from fla.ops.nda.chunk_fwd import chunk_nda_fwd
from fla.ops.utils.index import prepare_chunk_indices
from fla.utils import autocast_custom_bwd, autocast_custom_fwd, input_guard


class ChunkNDAFunction(torch.autograd.Function):
    @staticmethod
    @input_guard
    @autocast_custom_fwd
    def forward(
        ...
        (o, final_state, g_cumsum, Aqk, Akk, w, u, qg, kg, v_new, h, initial_state) = chunk_nda_fwd(
            q=q,
            k=k,
            v=v,
            g=g_input,
    ...
    def backward(
        ...
        dq, dk, dv, db, dg, dh0, dA, dbias = chunk_nda_bwd(
            q=q,
            k=k,
            v=v,
            g=g_cumsum,
    ...
    **kwargs,
    ...
    Args:
        ...
        scale (Optional[float]):
            Scale factor for the NDA attention scores.
        ...
        use_gate_in_kernel (bool):
            Whether to compute the log-space NDA decay internally.
            - If `True`:
    ...
    Examples::
        ...
        >>> from fla.ops.nda import chunk_nda
    ...
    assert k.shape[-1] <= 256, "Currently we only support key headdim <=256 for NDA :-("
    assert beta.shape == q.shape, "beta must be of shape (batch size, seq len, num of head, key dim)."
    assert v.shape == (*q.shape[:3], v.shape[-1]), "v must be of shape (batch size, seq len, num of head, head dim)."

    if scale is None:
        scale = k.shape[-1] ** -0.5
    return ChunkNDAFunction.apply(
        q,
        k,
        v,
        g,
