...
from fla.ops.nda.chunk_intra import chunk_nda_bwd_intra
...
@triton.jit(do_not_specialize=['T'])
def chunk_nda_bwd_kernel_wy_dqkg_fused(
    q,
    k,
    v,
    v_new,
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        p_beta = tl.make_block_ptr(beta, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
        b_beta = tl.load(p_beta, boundary_check=(0, 1)).to(tl.float32)         
        ...
        b_dkh = b_dk * tl.where(m_t[:, None], exp2(b_gn[None, :] - b_g), 0)
...
def chunk_nda_bwd_wy_dqkg_fused(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    v_new: torch.Tensor,
    ...
    grid = (NT, B * H)
    chunk_nda_bwd_kernel_wy_dqkg_fused[grid](
        q=q,
        k=k,
        v=v,
        v_new=v_new,
...
def chunk_nda_bwd(
    ...
    **kwargs,
    ...
    dq, dk, dv, db, dg, dAkk = chunk_nda_bwd_wy_dqkg_fused(
        q=q,
        k=k,
        v=v,
        v_new=v_new,
    ...
    dq, dk, db, dg = chunk_nda_bwd_intra(
        q=q,
        k=k,
        g=g,
        beta=beta,
