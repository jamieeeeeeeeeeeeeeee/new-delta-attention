...
from fla.ops.nda.chunk_intra_token_parallel import chunk_nda_fwd_intra_token_parallel
from fla.ops.nda.wy_fast import recompute_w_u_fwd
...
def chunk_nda_fwd_kernel_inter_solve_fused(
    q,
    k,
    g,
    beta,
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        p_b0 = tl.make_block_ptr(beta, (T, K), (H*K, 1), (i_tc0, i_k * BK), (BC, BK), (1, 0))
        b_k0 = tl.load(p_k0, boundary_check=(0, 1)).to(tl.float32)
        b_g0 = tl.load(p_g0, boundary_check=(0, 1)).to(tl.float32)
        b_kb0 = b_k0 * tl.load(p_b0, boundary_check=(0, 1)).to(tl.float32)
        ...
        if i_tc1 < T:
            ...
            p_b1 = tl.make_block_ptr(beta, (T, K), (H*K, 1), (i_tc1, i_k * BK), (BC, BK), (1, 0))
            # [BC, BK]
            ...
            b_kb1 = tl.load(p_b1, boundary_check=(0, 1)).to(tl.float32) * b_k1
            # [BK]
            ...
            b_kgt = tl.trans(b_kb0 * exp2(b_gn1[None, :] - b_g0))
            # [BC, BC]
            ...
            if i_tc2 < T:
                ...
                p_b2 = tl.make_block_ptr(beta, (T, K), (H*K, 1), (i_tc2, i_k * BK), (BC, BK), (1, 0))
                # [BC, BK]
                ...
                b_kb2 = b_k2 * tl.load(p_b2, boundary_check=(0, 1)).to(tl.float32)
                # [BK]
...
def chunk_nda_bwd_kernel_intra(
    q,
    k,
    g,
    beta,
    ...
    p_b = tl.make_block_ptr(beta, (T, K), (H*K, 1), (i_ti, i_k * BK), (BC, BK), (1, 0))
    ...
    if i_i > 0:
        ...
        for i_j in range(0, i_i):
            ...
            p_bj = tl.make_block_ptr(beta, (T, K), (H*K, 1), (i_t * BT + i_j * BC, i_k * BK), (BC, BK), (1, 0))
            ...
            b_k = b_k * tl.load(p_bj, boundary_check=(0, 1))
    ...
    if SAFE_GATE:
        ...
        #b_k_exp_diag_qk = b_k * exp_neg_b_g_diag_qk
        b_k_exp_diag_qk = b_k * b_b * exp_neg_b_g_diag_qk
        b_dq2 += tl.dot(b_dAqk_diag_qk, b_k_exp_diag_qk) * exp_b_g_diag_qk
        b_dk2 += tl.dot(b_dAkk_diag_qk, b_k_exp_diag_qk) * exp_b_g_diag_qk

    else:
        #p_bj = beta + i_ti * H
        p_bj = beta + i_ti * H * K + o_k
        for j in range(0, min(BC, T - i_t * BT - i_i * BC)):
            ...
            b_kj = tl.load(p_kj, mask=m_k, other=0).to(tl.float32)
            b_kj = b_kj * tl.load(p_bj, mask=m_k, other=0).to(tl.float32)
    ...
    if i_i < NC - 1:
        ...
        for i_j in range(i_i + 1, NC):
            ...
            b_kb = tl.load(p_k, boundary_check=(0, 1))
    ...
    if SAFE_GATE:
        ...
        for j in range(0, min(BC, T - i_t * BT - i_i * BC)):
            ...
            b_kbj = tl.load(p_kj, mask=m_k, other=0).to(tl.float32)
            b_gkj = tl.load(p_gkj, mask=m_k, other=0).to(tl.float32)
            # [BC, BK]
            m_i = o_i[:, None] <= j
    ...
    p_db = tl.make_block_ptr(db, (T, K), (H*K, 1), (i_ti, i_k * BK), (BC, BK), (1, 0))

    #b_dg2 += (b_dk2 - b_dkt) * b_k + tl.load(p_dg, boundary_check=(0, 1))
    b_dg2 += (b_dk2 - b_dkt * b_b) * b_k + tl.load(p_dg, boundary_check=(0, 1))
    b_db = b_dkt * b_k
...
@triton.jit(do_not_specialize=['T'])
def chunk_nda_fwd_kernel_intra_sub_chunk(
    q,
    k,
    g,
    beta,
    ...
    p_beta = tl.make_block_ptr(beta, (T, K), (H*K, 1), (i_ti, 0), (BC, BK), (1, 0))

    b_q = tl.load(p_q, boundary_check=(0, 1))
    b_k = tl.load(p_k, boundary_check=(0, 1))
    b_g = tl.load(p_g, boundary_check=(0, 1))
    #b_beta = tl.load(p_beta, boundary_check=(0,))
    b_beta = tl.load(p_beta, boundary_check=(0, 1))
    ...
    b_kgt = tl.trans(b_k * b_beta * b_gk)
...
def chunk_nda_fwd_intra(
    ...
    if safe_gate:
        ...
        chunk_nda_fwd_kernel_intra_sub_chunk[grid](
            q=q,
            k=k,
            g=gk,
            beta=beta,
        ...
        Aqk, Akkd = chunk_nda_fwd_intra_token_parallel(
            q=q,
            k=k,
            gk=gk,
            beta=beta,
    ...
    grid = (NT, B * H)
    chunk_nda_fwd_kernel_inter_solve_fused[grid](
        q=q,
        k=k,
        g=gk,
        beta=beta,
...
def chunk_nda_bwd_intra(
    ...
    db2 = beta.new_zeros(NK, *beta.shape, dtype=torch.float)
    dg2 = torch.zeros_like(dg, dtype=torch.float)
    grid = (NK * NC, NT, B * H)
    chunk_nda_bwd_kernel_intra[grid](
        q=q,
        k=k,
        g=g,
        beta=beta,
