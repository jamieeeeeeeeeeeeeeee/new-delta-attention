...
def fused_recurrent_nda_fwd_kernel(
    q,
    k,
    v,
    g,
    ...
    for i_t in tl.range(0, T, num_stages=num_stages):
        ...
        if IS_BETA_HEADWISE:
            ...
            b_beta = tl.load(p_beta, mask=mask_k, other=0, eviction_policy='evict_last').to(tl.float32)
        else:
            # Not updated for NDA
        ...
        if TRANSPOSE_STATE:
            ...
            b_h += b_v[:, None] * b_kb[None, :]
            b_o = tl.sum(b_h * b_q[None, :], 1)
        else:
            #b_h += b_k[:, None] * b_v[None, :]
            b_h += b_kb[:, None] * b_v[None, :]
        ...
        p_beta += HV * (K if IS_BETA_HEADWISE else 1)
...
def fused_recurrent_nda_fwd(
    ...
    **kwargs,
    ...
    fused_recurrent_nda_fwd_kernel[grid](
        q=q,
        k=k,
        v=v,
        g=g,
...
def fused_recurrent_nda(
    ...
    **kwargs,
    ...
    Examples::
        ...
        >>> from fla.ops.nda import fused_recurrent_nda
        ...
        >>> o_var, ht_var = fused_recurrent_nda(
            q, k, v, g, beta,
            initial_state=h0,
            output_final_state=True,
            cu_seqlens=cu_seqlens
    ...
    if g.shape[:3] != v.shape[:3] or g.shape[-1] != q.shape[-1]:
        raise ValueError(
            f"`g` must have shape [B, T, HV, K] compatible with `v` and `q`, got {g.shape}."
        )
    if beta.shape != g.shape:
        raise ValueError(
            f"`beta` must have the same shape as `g` for NDA, got {beta.shape} and {g.shape}."
        )
    if scale is None:
        scale = k.shape[-1] ** -0.5

    o, final_state = fused_recurrent_nda_fwd(
        q=q,
        k=k,
        v=v,
        g=g,
