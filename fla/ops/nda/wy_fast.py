...
def recompute_w_u_fwd_nda_kernel(
    q,
    k,
    qg,
    kg,
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        p_b = tl.make_block_ptr(beta + (bos*H + i_h) * K, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
        b_beta = tl.load(p_b, boundary_check=(0, 1))
        #b_kb = b_k * b_b[:, None]
        b_kb = b_k * b_beta
        ...
        if STORE_KG:
            ...
            b_kgl = b_kb * tl.where((i_t * BT + tl.arange(0, BT) < T)[:, None], exp2(b_gn[None, :] - b_gk), 0)
            p_kg = tl.make_block_ptr(kg + (bos * H + i_h) * K, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            tl.store(p_kg, b_kgl.to(p_kg.dtype.element_ty), boundary_check=(0, 1))

        #b_w = tl.dot(b_A, b_kb.to(b_k.dtype))
        b_w = tl.dot(b_A, b_kgr.to(b_k.dtype))
...
@triton.jit(do_not_specialize=['T'])
def prepare_wy_repr_bwd_nda_kernel(
    k,
    v,
    beta,
    gk,
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        b_dA += tl.dot(b_dw, tl.trans(b_kg).to(b_dw.dtype))
        ...
        b_dk = b_dkbg * b_gk_exp + tl.load(p_dk, boundary_check=(0, 1))
        b_db = tl.zeros([BT, BK], dtype=tl.float32)
        b_dg = b_kg * b_dkbg + tl.load(p_dg, boundary_check=(0, 1))

        p_db = tl.make_block_ptr(db + (bos*H + i_h) * K, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
...
def recompute_w_u_fwd(
    ...
    recompute_w_u_fwd_nda_kernel[(NT, B*H)](
        q=q,
        k=k,
        qg=qg,
        kg=kg,
...
def prepare_wy_repr_bwd(
    ...
    dk2 = torch.zeros_like(dk, dtype=torch.float)
    dv = torch.zeros_like(v)
    dg2 = torch.zeros_like(gk, dtype=torch.float)
    dA = torch.zeros_like(A, dtype=torch.float)
    db = torch.zeros_like(beta, dtype=torch.float)
    prepare_wy_repr_bwd_nda_kernel[(NT, B * H)](
        k=k,
        v=v,
        beta=beta,
        gk=gk,
